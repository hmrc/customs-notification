...
import uk.gov.hmrc.customs.notification.services.{CustomsNotificationWorkItemService, DateTimeService}
...
class CustomsNotificationWorkItemController @Inject()(logger: NotificationLogger,
                                                      customsWorkItemNotificationService: CustomsNotificationWorkItemService,
  ...
  private def process(xml: NodeSeq, requestMetaData: RequestMetaData)(implicit hc: HeaderCarrier): Future[Result] = {
    logger.debug(s"Received notification with payload: $xml, metaData: $requestMetaData")
