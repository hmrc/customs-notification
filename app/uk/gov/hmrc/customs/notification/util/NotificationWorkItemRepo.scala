...
import uk.gov.hmrc.customs.notification.models.repo._
...
class NotificationWorkItemRepo @Inject()(mongo: MongoComponent,
                                         ...
                                         logger: CdsLogger,
                                         configuration: Configuration)(implicit ec: ExecutionContext) extends WorkItemRepository[NotificationWorkItem](
  ...
  def saveWithLock(notificationWorkItem: NotificationWorkItem, processingStatus: CustomProcessingStatus = SuccessfullyCommunicated): Future[WorkItem[NotificationWorkItem]] = {
    logger.debug(s"saving a new notification work item in locked state (${processingStatus.name}) $notificationWorkItem")
    val workItem: WorkItem[NotificationWorkItem] = new WorkItem[NotificationWorkItem](
      id = new ObjectId(),
      receivedAt = now(),
      updatedAt = now(),
      availableAt = now(),
      status = processingStatus.convertToHmrcProcessingStatus,
      failureCount = 0,
  ...
  def setFailedAndBlocked(id: ObjectId, httpStatus: Int): Future[Unit] = {
    logger.debug(s"setting completed status of ${FailedAndBlocked.name} for notification work item id: ${id.toString}")
  ...
  def unblockFailedAndBlockedByClientId(clientId: ClientId): Future[Int] = {
  ...
  def unblockFailedAndBlockedByCsId(csid: ClientSubscriptionId): Future[Int] = {
    ...
    collection.updateMany(selector, update).toFuture().map { result =>
      logger.debug(s"updated ${result.getModifiedCount} notifications with status equal to ${FailedAndBlocked.name} to ${FailedButNotBlocked.name} for csid ${csid.id}")
      result.getModifiedCount.toInt
    }
  }
  ...
  def failedAndBlockedWithHttp5xxByCsIdExists(csid: ClientSubscriptionId): Future[Boolean] = {
  ...
  def failedAndBlockedGroupedByDistinctCsId(): Future[Set[ClientSubscriptionId]] = {
    val selector = and(
      equal(workItemFields.status, FailedAndBlocked.convertToBson),
      lt("availableAt", now()))
  ...
  def pullOutstandingWithFailedWith500ByCsId(csid: ClientSubscriptionId): Future[Option[WorkItem[NotificationWorkItem]]] = {
    val selector = csIdAndStatusSelector(csid, FailedAndBlocked)
