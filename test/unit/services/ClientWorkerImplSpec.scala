  ...
  "ClientWorkerImpl" can {
    "for happy path" should {
      "push notifications when there are no errors" in new SetUp {
        ...
        when(mockDeclarantDetails.getClientData(ameq(CsidOne.id.toString))(any[HeaderCarrier])).thenReturn(Future.successful(Some(ApiSubscriptionFieldsResponseOne)), Future.successful(Some(ApiSubscriptionFieldsResponseTwo)))
        when(mockPush.send(ApiSubscriptionFieldsResponseOne, ClientNotificationOne)).thenReturn(true)
        when(mockPush.send(ApiSubscriptionFieldsResponseTwo, ClientNotificationTwo)).thenReturn(true)
        ...
        actual shouldBe (())
        eventually {
          ordered.verify(mockPush).send(ameq(ApiSubscriptionFieldsResponseOne), ameq(ClientNotificationOne))
          ordered.verify(mockRepo).delete(ClientNotificationOne)
          ordered.verify(mockPush).send(ameq(ApiSubscriptionFieldsResponseTwo), ameq(ClientNotificationTwo))
      ...
      "push notifications even when there are errors deleting the notification after a successful push" in new SetUp {
        ...
        when(mockLockRepo.release(eqClientSubscriptionId(CsidOne), eqLockOwnerId(CsidOneLockOwnerId))).thenReturn(Future.successful(()))
        when(mockDeclarantDetails.getClientData(ameq(CsidOne.id.toString))(any[HeaderCarrier])).thenReturn(Future.successful(Some(ApiSubscriptionFieldsResponseOne)))
        when(mockPush.send(ApiSubscriptionFieldsResponseOne, ClientNotificationOne)).thenReturn(true)
      ...
      "exit push processing when fetch returns same size of list twice in a row, when declarant details not found and pull queue fails" in new SetUp {
        ...
        when(mockDeclarantDetails.getClientData(ameq(CsidOne.id.toString))(any[HeaderCarrier])).thenReturn(Future.successful(Some(ApiSubscriptionFieldsResponseOne)), Future.successful(None))
        when(mockLockRepo.release(eqClientSubscriptionId(CsidOne), eqLockOwnerId(CsidOneLockOwnerId))).thenReturn(Future.successful(()))
        when(mockPush.send(ApiSubscriptionFieldsResponseOne, ClientNotificationOne)).thenReturn(false)
      ...
      "enqueue notification to pull queue when declarant callback URL is empty" in new SetUp {
        ...
        when(mockDeclarantDetails.getClientData(ameq(CsidOne.id.toString))(any[HeaderCarrier])).thenReturn(Future.successful(Some(ApiSubscriptionFieldsResponseOneWithEmptyUrl)))
        when(mockPull.send(ameq(ClientNotificationOne))).thenReturn(true)
        when(mockRepo.delete(ameq(ClientNotificationOne))).thenReturn(Future.successful(()))
        when(mockLockRepo.release(eqClientSubscriptionId(CsidOne), eqLockOwnerId(CsidOneLockOwnerId))).thenReturn(Future.successful(()))
        when(mockConfig.pullExcludeConfig).thenReturn(mockPullExcludeConfig)
