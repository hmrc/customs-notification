  ...
  "ClientWorkerImpl" can {
    "for happy path" should {
      "push notifications when there are no errors" in new SetUp {
        ...
        actual shouldBe (())
        eventually {
          ordered.verify(mockPush).send(ameq(ApiSubscriptionFieldsOne), ameq(ClientNotificationOne))
          ordered.verify(mockRepo).delete(ClientNotificationOne)
          ordered.verify(mockPush).send(ameq(ApiSubscriptionFieldsTwo), ameq(ClientNotificationTwo))
