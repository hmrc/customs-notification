...
class AppConfigSpec extends UnitSpec with MockitoSugar with Matchers {
...
//      actual.notificationQueueConfig shouldBe NotificationQueueConfig("http://localhost:9648/queue")
  ...
  private def configService(conf: Configuration) =
    new AppConfig(new ConfigValidatedNelAdaptor(testServicesConfig(conf), conf), stubCdsLogger)
