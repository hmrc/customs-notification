...
class CustomsNotificationClientWorkerServiceSpec extends UnitSpec with MockitoSugar with BeforeAndAfterEach with Eventually {
  ...
  private val requestMetaData = RequestMetaData(clientSubscriptionId, conversationId, Some(BadgeId(badgeIdValue)), Some(Submitter(submitterNumber)), Some(CorrelationId(correlationId)), TimeReceivedZoned)
  ...
  private val submitterIdHeader: (String, String) = hc.headers.filter(a => a._1 == X_SUBMITTER_ID_HEADER_NAME).head
  private val correlationIdHeader: (String, String) = hc.headers.filter(a => a._1 == X_CORRELATION_ID_HEADER_NAME).head
  private val expectedHeaders = Seq(Header(badgeIdHeader._1, badgeIdHeader._2), Header(submitterIdHeader._1, submitterIdHeader._2), Header(correlationIdHeader._1, correlationIdHeader._2))
