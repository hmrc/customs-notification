...
class PushNotificationRequestServiceSpec extends UnitSpec with MockitoSugar {
  ...
  val metaData = RequestMetaData(clientSubscriptionId, conversationId, Some(BadgeId(badgeId)),
  ...
  "PushNotificationRequestService" should {
    ...
    "return valid request when badgeId is provided" in {
      val metaDataWithSomeBadgeId = RequestMetaData(clientSubscriptionId, conversationId, Some(BadgeId(badgeId)), None,
    ...
    "return valid request when submitterNumber is provided" in {
      val metaDataWithSomeSubmitterNumber = RequestMetaData(clientSubscriptionId, conversationId, None, Some(Submitter(submitterNumber)),
        None, Some(FunctionCode(functionCode)), Some(IssueDateTime(issueDateTime)), Some(Mrn(mrn)), testDate)
