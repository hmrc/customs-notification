...
/**
  * this class is copied from customs-api-common
  * it was necessary, because of changing mockito dependency here
  * meaning:
  * in order to mock one of MongoDb related fields (collection), mockito-inline instead of mockito-core must be used
  * it changes the way mocks are created under the hood and caused binary conflict with helper class from customs-api-common
  * for the time being, this class has been copied over here to avoid potential impact on other components should mockito dependence
  * have been changed in customs-api-common
  *
  * separate ticket will be created in order to resolve this class duplication
  */
